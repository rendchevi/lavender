...
from utils import Dataset, ContextAwareDataCollator, ObjectConceptElaborationDataset
...
class ViPE(LightningModule):
    ...
    def setup(self, stage=None):
        if self.params.dataset_type == "vipe":
        ...
        elif self.params.dataset_type == "concept_object":
            # Assign train/val datasets for use in dataloaders
            self.train_dataset = ObjectConceptElaborationDataset(
                self.params.train_dataset_path,
                obj_col="object_list", elab_col="verbalization", concept_list=self.params.concept_list,
            )
            self.valid_dataset = ObjectConceptElaborationDataset(
                self.params.eval_dataset_path,
                obj_col="object_list", elab_col="verbalization", concept_list=self.params.concept_list,
            )
            self.data_collator = ContextAwareDataCollator(self.tokenizer)
