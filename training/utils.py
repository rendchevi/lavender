...
class ObjectConceptElaborationDataset(Dataset):

    def __init__(self, datapath, obj_col, elab_col, concept_list, sep="\t"):

        self.obj_col = obj_col
        self.elab_col = elab_col

        self.concept_list = concept_list

        self.data = pd.read_csv(datapath, sep=sep)

    def __len__(self):
        return len(self.data)

    def __getitem__(self, idx):
        objs = self.data.iloc[idx][self.obj_col]
        
        concepts = [e for e in self.concept_list if (self.data.iloc[idx][self.concept_list] > 0)[e]]
        concepts = np.random.choice(concepts, size=random.randint(1,len(concepts)), replace=False).tolist()
        concepts = ", ".join(concepts)

        elab = self.data.iloc[idx][self.elab_col]

        context = f"[concepts] {concepts} [objects] {objs}"
        prompt = elab
